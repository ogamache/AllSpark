...
class SemiDatasetWeak(Dataset):
    ...
    def __getitem__(self, item):
        ...
        if self.mode == 'val':
            transformed_val = self.transform_normalize(image=np.array(img), mask=np.array(mask))
        ...
        if self.mode == 'train_u':
            transformed_unlabeled = self.transform_normalize(image=np.array(img))
        ...
        transformed = self.transform_data_augmentation(image=np.array(img_tf), mask=np.array(mask_tf))
